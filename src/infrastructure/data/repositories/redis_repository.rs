...
impl RedisRepository {
    ...
    pub async fn set_auth_data(
        ...
        self.redis_client
            .set(&user_data_key, json!(data).to_string())
            .await?;
    ...
    pub async fn get_user_data(&mut self, key: &String) -> Option<UserWithRoles> {
        ...
        let user_data: Option<String> = self.redis_client.get(&user_data_key).await.ok();

        if let Some(u) = user_data {
            let u_data: UserWithRoles = serde_json::from_str(&u)
                .map_err(|err| {
                    debug!("[Some(u)] error parsing user data: {}", err);
                    err
                })
                .ok()?;
